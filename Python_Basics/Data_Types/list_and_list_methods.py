...
might contain items of different data types, but usually the items all have
the same type (it's easy to iterate over them).
"""
...
def list_type():
  ...
  # indexed and sliced:
  assert squares[0] # output: 1  # indexing returns the item
  assert squares[-1] # output: 25
  assert squares[-3:] # output: [9, 16, 25]  # slicing returns a new list

  # All slice operations return a new list containing the requested elements.
  # This means that the following slice returns a new (shallow) copy of
  # the list:
  assert squares[:] # output: [1, 4, 9, 16, 25]

  # Lists also support operations like concatenation:
  assert squares + [36, 49, 64, 81, 100] # output: [1, 4, 9, 16, 25, 36, 49, 64, 81, 100]

  # Unlike strings, which are immutable, lists are a mutable type, i.e. it
  # is possible to change their content:
  cubes = [1, 8, 27, 65, 125]  # something's wrong here, the cube of 4 is 64!
  cubes[3] = 64  # replace the wrong value
  assert cubes # output: [1, 8, 27, 64, 125]

  # You can also add new items at the end of the list, by using
  # the append() method
  cubes.append(216)  # add the cube of 6
  cubes.append(7 ** 3)  # and the cube of 7
  assert cubes # output: [1, 8, 27, 64, 125, 216, 343]

  # Assignment to slices is also possible, and this can even change the size
  # of the list or clear it entirely:
  letters = ['a', 'b', 'c', 'd', 'e', 'f', 'g']
  letters[2:5] = ['C', 'D', 'E']  # replace some values
  assert letters # output: ['a', 'b', 'C', 'D', 'E', 'f', 'g']
  letters[2:5] = []  # now remove them
  assert letters # output: ['a', 'b', 'f', 'g']
  # clear the list by replacing all the elements  = []
  assert letters # output: [  

  # The built-in function len() also applies to lists
  letters = ['a', 'b', 'c', 'd']
  assert len(letters) # output: 4
  ...
  assert mixed_list # output: [['a', 'b', 'c'], [1, 2, 3]]
  assert mixed_list[0] # output: ['a', 'b', 'c']
  assert mixed_list[0][1] # output: 'b'
...
def test_list_methods():
  ...
  fruits.append('grape')
  assert fruits # output: ['orange', 'apple', 'pear', 'banana', 'kiwi', 'apple', 'banana', 'grape']

  # list.remove(x)
  # Remove the first item from the list whose value is equal to x.
  # It raises a ValueError if there is no such item.
  fruits.remove('grape')
  assert fruits # output: ['orange', 'apple', 'pear', 'banana', 'kiwi', 'apple', 'banana']
  ...
  assert fruits # output: ['grape', 'orange', 'apple', 'pear', 'banana', 'kiwi', 'apple', 'banana']
  ...
  assert fruits.index('grape') # output: 0
  assert fruits.index('orange') # output: 1
  assert fruits.index('banana') # output: 4
  assert fruits.index('banana', 5) # output: 7  # Find next banana starting a position 5
  ...
  assert fruits.count('tangerine') # output: 0
  assert fruits.count('banana') # output: 2

  # list.copy()
  # Return a shallow copy of the list. Equivalent to a[:].
  fruits_copy = fruits.copy()
  assert fruits_copy # output: ['grape', 'orange', 'apple', 'pear', 'banana', 'kiwi', 'apple', 'banana']

  # list.reverse()
  # Reverse the elements of the list in place.
  fruits_copy.reverse()
  assert fruits_copy # output: [ 'banana', 'apple', 'kiwi', 'banana', 'pear', 'apple', 'orange', 'grape']

  # list.sort(key=None, reverse=False)
  # Sort the items of the list in place (the arguments can be used for sort customization,
  # see sorted() for their explanation).
  fruits_copy.sort()
  assert fruits_copy # output: [ 'apple', 'apple', 'banana', 'banana', 'grape', 'kiwi', 'orange', 'pear']
  ...
  assert fruits # output: ['grape', 'orange', 'apple', 'pear', 'banana', 'kiwi', 'apple', 'banana']
  assert fruits.pop() # output: 'banana'
  assert fruits # output: ['grape', 'orange', 'apple', 'pear', 'banana', 'kiwi', 'apple']

  # list.clear()
  # Remove all items from the list. Equivalent to del a[:].
  fruits.clear()
  assert fruits # output: []
...
def test_del_statement():
  ...
  del numbers[0]
  assert numbers # output: [1, 66.25, 333, 333, 1234.5]

  del numbers[2:4]
  assert numbers # output: [1, 66.25, 1234.5]
  ...
  assert numbers # output: []  # noqa: F821
...
def test_list_comprehensions():
  ...
  assert squares # output: [0, 1, 4, 9, 16, 25, 36, 49, 64, 81]
  ...
  assert combinations # output: [(1, 3), (1, 4), (2, 3), (2, 1), (2, 4), (3, 1), (3, 4)]
  ...
  # Create a new list  = [x * 2 for x in vector]
#   assert doubled_vect  r # output: [-8, -4, 0, 4, 8]

  # Filter the list to exclude negative numbers.
  positive_vector = [x for x in vector if x >= 0]
  assert positive_vector # output: [0, 2, 4]

  # Apply a function to all the elements.
  abs_vector = [abs(x) for x in vector]
  assert abs_vector # output: [4, 2, 0, 2, 4]

  # Call a method on each element.
  fresh_fruit = ['  banana', '  loganberry ', 'passion fruit  ']
  clean_fresh_fruit = [weapon.strip() for weapon in fresh_fruit]
  assert clean_fresh_fruit # output: ['banana', 'loganberry', 'passion fruit']

  # Create a list of 2-tuples like (number, square).
  square_tuples = [(x, x ** 2) for x in range(6)]
  assert square_tuples # output: [(0, 0), (1, 1), (2, 4), (3, 9), (4, 16), (5, 25)]

  # Flatten a list using a listcomp  = [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
  flatten_vector = [num for elem in  vector for num in elem]
  assert flatten_vector # output: [1, 2, 3, 4, 5, 6, 7, 8, 9]
...
def test_nested_list_comprehensions():
  ...
  assert transposed_matrix # output: [[1, 5, 9], [2, 6, 10], [3, 7, 11], [4, 8, 12]]
  ...
  assert transposed # output: [ [1, 5, 9], [2, 6, 10], [3, 7, 11], [4, 8, 12]]
  ...
  assert list(zip(*matrix)) # output: [ (1, 5, 9), (2, 6, 10), (3, 7, 11), (4, 8, 12)]